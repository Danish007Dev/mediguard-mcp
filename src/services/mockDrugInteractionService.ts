...
const MOCK_INTERACTIONS: MockInteractionDefinition[] = [
  {
    ...
    evidence: 'Mock evidence: representative of common high-risk interaction classes.'
  },
  {
    ...
    evidence: 'Mock evidence: representative of severe metabolism-mediated interactions.'
  }
];
...
function inferRiskLevel(interactions: InteractionFinding[]): RiskLevel {
  if (interactions.some((interaction) => interaction.severity === 'contraindicated')) {
    return 'critical';
  }
...
function findInteractions(medications: string[]): InteractionFinding[] {
  const medsSet = new Set(medications.map((medication) => medication.toLowerCase()));
